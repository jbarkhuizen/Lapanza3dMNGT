...
- **`src/api/client.ts`** — a thin fetch wrapper: `apiGet(path)`, `apiPost(path, body)`, `apiPatch(path, body)`. Always `credentials: 'include'`. Base URL: `import.meta.env.VITE_API_BASE_URL` (dev: `http://localhost:4200`; prod build: `/api`, since same-origin path-based hosting needs no host at all). Throws a typed `ApiError { status, message }` on any `{ ok: false }` response, so pages don't each hand-roll error parsing.
