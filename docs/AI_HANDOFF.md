...
| **Frontend** | **`platform/frontend/` exists and is deployed live** at `https://barkie.co.za/app/` (React + Vite + TS + Tailwind + React Router + React Query SPA, static build served by nginx). Done: Login/Register/Verify-Email, authenticated app shell (nav + logout), dashboard home placeholder, 404 page, **Company Profile**, **Customers**, **Filaments**, **Labour Steps**, **Consumables**, **Printers** (+ nested Presets/Maintenance Logs) — every reference-data module now has a full CRUD UI. 118 tests passing. Costing Templates, Quotes, Invoices still have no UI — next up, per the frontend design spec's phased execution order. **Read the "Frontend form-data gotcha" note below before building any more CRUD forms** — this discipline caught 5 real bugs across the 4 reference-data phases (blank-vs-null-vs-omitted mishandling, a required-number field silently coercing "cleared" to "0", and — new in the Printers phase — `<input type="number">` rejecting decimal values in real browsers because jsdom doesn't enforce HTML step validation, now fixed app-wide in `FormField.tsx` with a default `step="any"`). Keep doing the schema-verification step every phase; don't skip it because recent modules didn't need it. |
