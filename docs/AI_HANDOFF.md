...
| Item | State |
|---|---|
| **barkie.co.za (live domain)** | Currently live: the old `landing/` coming-soon page (still deployed as of this handoff). **Code-complete but not yet deployed:** a real 3-page public marketing site (Home, Pricing, Materials Guide) that replaces it, built 2026-09-09 across an 8-task plan (see `docs/superpowers/specs/2026-09-09-public-site-design.md`). Deploying it requires shipping `landing/`'s new pages *and* `platform/api`'s two new public endpoints together (see the `landing/` row below) — do that before considering barkie.co.za's public face "done". The systemd service `barkie-landing.service` on the VPS (`/opt/barkie/app`, `node server.js`, port 4100, `User=deploy`, `Restart=on-failure`) and nginx reverse-proxy (`/etc/nginx/conf.d/barkie.conf`, `barkie.co.za`/`www.barkie.co.za` → it) don't change — only the static content it serves does. Existing Certbot SSL cert untouched. |
| **`landing/`** | 3 pages, code-complete, not yet deployed: `public/index.html` (Home — hero + live usage stats strip), `public/pricing.html` (Pricing — 3 plan cards, R25/R45/R70), `public/materials.html` (Materials Guide — 3-tab Selector/All materials/Head-to-head, 28-material browsable library). Home and Pricing are **not self-contained** — they fetch `platform/api`'s two public, read-only endpoints at runtime (`GET /api/public/stats`, `GET /api/public/plans`); deploy both sides together or the stats strip/pricing grid show hidden/fallback state. Materials Guide has no such dependency — its dataset is static, hand-curated content in `landing/public/js/materials-data.js` (28 materials, South African retail prices) that will need re-verification if filament prices drift meaningfully over time; it is not sourced from any API or database. Deploy access: `ssh -i ~/.ssh/lapanza_vps_deploy deploy@41.222.36.147` (same key as lapanza3d; `deploy` has passwordless sudo on this box). To redeploy after a code change: `tar` the `landing/` folder (excluding `node_modules`/`data`/`.env`), `scp` it up, extract into `/opt/barkie/app`, `npm install --omit=dev`, `sudo systemctl restart barkie-landing`. |
...
2. Decide what to do about the one existing prod tenant
   ...
   free/test account.
3. **Deploy the new barkie.co.za public site** — the 3-page marketing site
   (Home, Pricing, Materials Guide) that replaces the coming-soon
   placeholder is code-complete (see the `landing/` row in "Current state"
   above) but not yet deployed. Deploy `landing/`'s new pages and
   `platform/api`'s two new public endpoints together, then smoke-test:
   all 3 pages load, the stats strip and pricing cards show real data,
   and the Materials Guide Selector genuinely narrows results for at
   least 2 different printer profiles.
