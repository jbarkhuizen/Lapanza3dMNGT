...
**Phase 1 (per the original phased design spec) is now complete end to end** — every planned backend module, every frontend page, and PDF generation + dev-mode email sending are all built, tested, merged, and deployed live. There is no more "next slice" of the originally-scoped platform left to build; what remains is:

1. Real SMTP for email sending (still dev-mode only — `sendDocumentEmail()` logs to `journalctl -u barkie-api` instead of sending). Needs actual mail credentials as a dependency (SRS §2.3 names cPanel SMTP).
2. Billing/subscription (Phase 2, not started) — needs PayFast + PayPal merchant credentials as a dependency.
3. The backlog board's accumulated Low-priority polish items (#30-57 as of this handoff) — none urgent, worth a look before onboarding a real first tenant.
