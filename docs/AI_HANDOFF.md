...
| **Frontend** | **`platform/frontend/` exists and is deployed live** at `https://barkie.co.za/app/` (React + Vite + TS + Tailwind + React Router + React Query SPA, static build served by nginx). So far: Login/Register/Verify-Email, authenticated app shell (nav + logout), dashboard home placeholder, 404 page, **Company Profile**, **Customers**, **Filaments**, **Labour Steps**, **Consumables** (all list/create/edit) — 96 tests passing. Printers (+ nested Presets/Maintenance Logs), Costing Templates, Quotes, Invoices still have no UI — next up, per the frontend design spec's phased execution order. **Read the "Frontend form-data gotcha" note below before building any more CRUD forms** — Company Profile shipped completely broken (every save 400'd) before a review caught it; the Filaments/Labour Steps/Consumables phase deliberately re-verified the same risk per-module and caught two more real bugs (a date-format field needing special blank-handling, and a required-number field silently coercing "cleared" to "0") — this discipline works, keep doing it every phase, don't skip the schema-verification step because "the last two modules didn't need it." |
