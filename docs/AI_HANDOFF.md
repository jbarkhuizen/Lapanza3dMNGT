...
| **Frontend** | **`platform/frontend/` exists and is deployed live** at `https://barkie.co.za/app/` (React + Vite + TS + Tailwind + React Router + React Query SPA, static build served by nginx). So far: Login/Register/Verify-Email, authenticated app shell (nav + logout), dashboard home placeholder, 404 page, **Company Profile** (settings form, all 19 fields) and **Customers** (list/create/edit) — 63 tests passing. Printers, Filaments, Labour Steps, Consumables, Costing Templates, Quotes, Invoices still have no UI — next up, per the frontend design spec's phased execution order. **Read the "Frontend form-data gotcha" note below before building any more CRUD forms** — Company Profile shipped completely broken (every save 400'd) before a review caught it, and the root cause will recur in every remaining phase if not handled deliberately. |
