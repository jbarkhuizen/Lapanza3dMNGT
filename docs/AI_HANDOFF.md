...
**Phase 1 (per the original phased design spec) is now complete end to end, including real email** — every planned backend module, every frontend page, PDF generation, and real SMTP sending are all built, tested, merged, and deployed live. There is no more "next slice" of the originally-scoped platform left to build; what remains is:

1. Billing/subscription (Phase 2, not started) — needs PayFast + PayPal merchant credentials as a dependency.
2. A resend-verification endpoint (backlog item #001) — the one real gap the SMTP work surfaced: a failed/lost verification email still has no self-service recovery path.
3. The backlog board's accumulated Low-priority polish items (#30-59 as of this handoff) — none urgent, worth a look before onboarding a real first tenant.
4. Redeploy both API and frontend after any future code change (see "Deploying" below) — migrations apply automatically to `barkie_prod` via `prisma migrate deploy` whenever a future change adds one.
