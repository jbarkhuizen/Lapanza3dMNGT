...
| **Frontend** | **`platform/frontend/` exists and is deployed live** at `https://barkie.co.za/app/` (React + Vite + TS + Tailwind + React Router + React Query SPA, static build served by nginx). Done: Login/Register/Verify-Email, authenticated app shell (nav + logout), dashboard home placeholder, 404 page, every reference-data module (**Company Profile**, **Customers**, **Filaments**, **Labour Steps**, **Consumables**, **Printers** + nested Presets/Maintenance Logs), and **Costing Templates** (list, read-only detail, create form with dynamic labour/consumable line items — the first page to actually DISPLAY Decimal money fields, via a new shared `formatCurrency()` helper in `src/lib/formatCurrency.ts`). 135 tests passing. Quotes, Invoices still have no UI — next up. **Read the "Frontend form-data gotcha" note below before building any more CRUD forms** — this discipline has caught 6 real bugs across 5 phases so far. Keep doing the schema-verification step every phase. |
