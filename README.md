...
|---|---|---|
| [`landing/`](landing/) | The public marketing site for barkie.co.za — 3 pages (Home, Pricing, Materials Guide) served by an Express app; Home/Pricing depend on two `platform/api` public endpoints, the Materials Guide runs on a static hand-curated dataset | Built and **live** at https://barkie.co.za/ as of 2026-09-09 (`barkie-landing.service` on the VPS), replacing the old coming-soon page; see `docs/AI_HANDOFF.md`. |
| [`platform/api/`](platform/api/) | The real product: Node/TypeScript/Express/Postgres API — auth, tenant isolation, Customer/Printer/Filament/Labour/Consumable CRUD, costing engine, Company Profile, Quotes, Invoices, PDF generation + real email sending, and billing/subscription (PayFast + PayPal) | Merged to `master`, pushed to GitHub, **live** at https://barkie.co.za/api/ (`barkie-api.service` on the VPS, `barkie_prod` Postgres DB). **Email sends for real via Gmail SMTP** as of 2026-09-08. **Billing routes and the subscription gate are live as of 2026-09-09, but payment-provider credentials aren't configured yet** — checkout will error until PayFast/PayPal credentials are added; see `docs/AI_HANDOFF.md`. |
| [`platform/frontend/`](platform/frontend/) | The subscriber dashboard SPA — React/Vite/TS/Tailwind, talks to `platform/api/` | Merged to `master`, pushed to GitHub, **live** at https://barkie.co.za/app/. Every module has a full UI, including Quotes/Invoices, a "Send to Customer" button, and plan-selection/checkout/billing-settings pages. **Phase 1 is complete end to end; Phase 2 (billing) is deployed but not yet usable pending real payment-provider credentials** — see `docs/AI_HANDOFF.md` for what's next. |
...
- **Domain:** barkie.co.za — live, serving the real public site + API + app (see the table above)
