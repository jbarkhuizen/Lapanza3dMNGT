...
| [`platform/frontend/`](platform/frontend/) | The subscriber dashboard SPA — React/Vite/TS/Tailwind, talks to `platform/api/` | Merged to `master`, pushed to GitHub, **live** at https://barkie.co.za/app/. Every module has a full UI, including Quotes and Invoices. PDF generation + email sending are the only remaining gap (dev-mode email only). |
