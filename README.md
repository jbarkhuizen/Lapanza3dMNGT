...
| [`platform/api/`](platform/api/) | The real product: Node/TypeScript/Express/Postgres API — auth, tenant isolation, Customer/Printer/Filament/Labour/Consumable CRUD, costing engine, Company Profile, Quotes, Invoices, and PDF generation + real email sending | Merged to `master`, pushed to GitHub, **live** at https://barkie.co.za/api/ (`barkie-api.service` on the VPS, `barkie_prod` Postgres DB). **Email sends for real via Gmail SMTP** as of 2026-09-08 (dev/test still stay console-log-only, automatically). |
| [`platform/frontend/`](platform/frontend/) | The subscriber dashboard SPA — React/Vite/TS/Tailwind, talks to `platform/api/` | Merged to `master`, pushed to GitHub, **live** at https://barkie.co.za/app/. Every module has a full UI, including Quotes/Invoices and a "Send to Customer" button that downloads a generated PDF and emails it for real. **Phase 1 (per the original design spec) is complete end to end** — see `docs/AI_HANDOFF.md` for what's next (billing, a resend-verification endpoint). |
