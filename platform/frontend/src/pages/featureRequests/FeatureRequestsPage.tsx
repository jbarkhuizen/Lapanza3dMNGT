...
function SubmitFeatureRequestForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 rounded border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800">
...
function MyFeatureRequestsSection() {
  ...
  return (
    <div className="flex flex-col gap-3 rounded border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800">
      ...
      {isError && <p className="text-red-600 dark:text-red-400">Couldn't load your requests. Try refreshing the page.</p>}
      {!isLoading && !isError && myRequests?.length === 0 && <p className="text-slate-500 dark:text-slate-400">No requests yet.</p>}
      {!isLoading && !isError && myRequests && myRequests.length > 0 && (
        <ul className="flex flex-col gap-3">
          {myRequests.map((request) => (
            <li key={request.id} className="flex flex-col gap-1 border-b border-slate-100 pb-3 last:border-b-0 last:pb-0 dark:border-slate-800">
              <div className="flex items-center justify-between gap-2">
                <span className="font-medium text-slate-900 dark:text-slate-100">{request.title}</span>
                <StatusBadge status={request.status} />
              </div>
              <span className="text-xs text-slate-500 dark:text-slate-400">{CATEGORY_LABELS[request.category]}</span>
...
function CommunityRequestRow({ request }: { request: CommunityFeatureRequest }) {
  ...
  return (
    <li className="flex items-start gap-4 border-b border-slate-100 pb-3 last:border-b-0 last:pb-0 dark:border-slate-800">
      <button
        type="button"
  ...
  return (
    <div className="flex flex-col gap-3 rounded border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Community requests</h2>
        <div className="flex gap-1 rounded bg-slate-100 p-1 text-sm dark:bg-slate-700">
          <button
            type="button"
            onClick={() => setView('vote')}
            className={`rounded px-3 py-1 ${view === 'vote' ? 'bg-white font-medium text-slate-900 shadow-sm dark:bg-slate-800 dark:text-slate-100' : 'text-slate-600 dark:text-slate-400'}`}
          >
            Vote
          </button>
          <button
            ...
            className={`rounded px-3 py-1 ${view === 'upcoming' ? 'bg-white font-medium text-slate-900 shadow-sm dark:bg-slate-800 dark:text-slate-100' : 'text-slate-600 dark:text-slate-400'}`}
          >
            Upcoming
          </button>
        </div>
      ...
      {isLoading && <p className="text-slate-500 dark:text-slate-400">Loading…</p>}
      {isError && <p className="text-red-600 dark:text-red-400">Couldn't load community requests. Try refreshing the page.</p>}
...
export function FeatureRequestsPage() {
  return (
    <div className="flex flex-col gap-6">
      <h1 className="text-2xl font-semibold text-slate-900 dark:text-slate-100">Feature Requests</h1>
