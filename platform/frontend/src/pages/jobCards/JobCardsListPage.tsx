  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-slate-900 dark:text-slate-100">Job Cards</h1>
        <div className="relative">
          ...
          {isTypeMenuOpen && (
            <div className="absolute right-0 z-10 mt-1 w-40 rounded border border-slate-200 bg-white py-1 shadow-md dark:border-slate-700 dark:bg-slate-800">
              {JOB_CARD_TYPES.map((type) => (
                <Link
                  key={type}
                  to={`/job-cards/new?type=${type}`}
                  onClick={() => setIsTypeMenuOpen(false)}
                  className="block px-3 py-2 text-sm text-slate-700 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-700"
                >
      ...
      {isError && <p className="text-red-600 dark:text-red-400">Couldn't load job cards. Try refreshing the page.</p>}
      {isLoading && <p className="text-slate-500 dark:text-slate-400">Loading…</p>}
      {!isLoading && !isError && jobCards?.length === 0 && <p className="text-slate-500 dark:text-slate-400">No job cards yet.</p>}
        ...
        <div className="overflow-x-auto">
          ...
          <tbody>
            {jobCards.map((jobCard) => (
              <tr key={jobCard.id} className="border-b border-slate-100 dark:border-slate-800">
                <td className="py-2">
                  <Link to={`/job-cards/${jobCard.id}`} className="text-slate-900 underline dark:text-slate-100">
                    {jobCard.number}
                  </Link>
                </td>
