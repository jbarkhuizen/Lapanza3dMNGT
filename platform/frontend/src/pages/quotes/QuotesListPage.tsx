  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-slate-900 dark:text-slate-100">Quotes</h1>
        <Link to="/quotes/new" className="rounded bg-slate-900 px-4 py-2 text-sm font-medium text-white dark:bg-slate-100 dark:text-slate-900">
          New Quote
        </Link>
      </div>
      ...
      {isError && <p className="text-red-600 dark:text-red-400">Couldn't load quotes. Try refreshing the page.</p>}
      {!isLoading && !isError && filteredQuotes?.length === 0 && <p className="text-slate-500 dark:text-slate-400">No quotes yet.</p>}
          ...
          <tbody>
            {filteredQuotes.map((quote) => (
              <tr key={quote.id} className="border-b border-slate-100 dark:border-slate-800">
                <td className="py-2">
                  <Link to={`/quotes/${quote.id}`} className="text-slate-900 underline dark:text-slate-100">
                    {quote.number}
                  </Link>
                </td>
                <td className="py-2">
