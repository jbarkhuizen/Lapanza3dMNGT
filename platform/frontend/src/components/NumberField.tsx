    ...
    <div className="flex flex-col gap-1">
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          className={`flex-1 rounded border border-slate-300 px-3 py-2 text-sm focus:border-slate-500 focus:outline-none dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 dark:focus:border-slate-400 ${className ?? ''}`}
        />
        {canClear && (
          <button
            type="button"
            ...
            title={`Clear ${label}`}
            className="rounded border border-slate-300 px-2 py-2 text-sm leading-none text-slate-500 hover:bg-slate-100 dark:border-slate-600 dark:text-slate-400 dark:hover:bg-slate-700"
          >
            {'\u00d7'}
          </button>
        )}
